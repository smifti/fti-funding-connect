...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    // กรณี Supabase ส่งมาแบบ ?code=... (PKCE) — ต้องแลก code เป็น session เอง
    const code = new URLSearchParams(window.location.search).get('code')
    if (code) {
      supabase.auth.exchangeCodeForSession(code).then(({ error }) => finish(!error))
    }

    // กรณี Supabase ส่งมาแบบ #access_token=...&type=recovery (hash fragment) —
    // ตัว SDK จะอ่าน hash นี้เองอัตโนมัติตอนโหลดหน้า แล้วยิง event 'PASSWORD_RECOVERY' ออกมา
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event) => {
      if (event === 'PASSWORD_RECOVERY') finish(true)
    })

    // เผื่อไว้กรณีอื่น (เช่น มี session อยู่แล้วจากการ refresh หน้า) — เช็คแบบปกติด้วย
    if (!code) {
      supabase.auth.getUser().then(({ data }) => { if (data.user) finish(true) })
    }

    // กันหน้าค้างที่ "กำลังตรวจสอบ..." ถ้าไม่มี code และไม่มี event ใดๆ เข้ามาเลย
